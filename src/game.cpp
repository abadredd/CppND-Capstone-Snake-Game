...
#include <chrono>             // For time durations in multithreading
...
Game::Game(std::size_t grid_width, std::size_t grid_height)
    : snake(grid_width, grid_height),
      ...
      powerUpThreadRunning(true) // Initialize powerUpThreadRunning here
{
    PlaceFood();
    powerUpThread = std::thread(&Game::PowerUpSpawnerThread, this);
}

// Join the thread in the destructor
Game::~Game()
{
    powerUpThreadRunning = false;
    powerUpCV.notify_one(); // Notify the thread to exit
    if (powerUpThread.joinable())
    {
        powerUpThread.join();
    }
}
...
void Game::PowerUpSpawnerThread()
{
    while (powerUpThreadRunning)
    {
        std::unique_lock<std::mutex> lock(powerUpMutex);
        powerUpCV.wait_for(lock, std::chrono::milliseconds(POWERUP_SPAWN_INTERVAL));

        if (!powerUpThreadRunning)
        {
            break; // Exit the loop if the thread is stopped
        }

        SpawnPowerUp();         // Call SpawnPowerUp here
        powerUpCV.notify_one(); // Notify the main thread
    }
}
